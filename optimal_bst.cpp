...
/*******************************************coding********/
ll level;
ll dp[100][100]={},root[100][100]={};// considering key from i to j dp[i][j];
...
int main()
{
    ...
    {
        ll n;cin>>n;
        vll keys(n+1,0),freq(n+1,0),pfreq(n+1,0);
        For(j,1,n) cin>>keys[j];
        For(j,1,n) cin>>freq[j] ,  pfreq[j]=(freq[j]+pfreq[j-1]);
        
        For(j,1,n) dp[j][j]=freq[j];
        For(len,1,n)
        {
            For(i,1,n-l+1)
            {
